...
public class BD 
{
    ...
    public static void  GuardarRecetas(Recetas Valor)
    {
        ...
        {
            db.Execute(sql, new {  pNombre = Valor.Nombre,pID=Valor.ID,  pIngredientes = Valor.Ingredientes, pFechaCreacion = Valor.FechaCreacion, pLikes=Valor.Likes, pImagen=Valor.Imagen, pNombreCreador=Valor.NombreCreador});
        } 
    ...
    public static int EliminarReceta(int ID)
    {
        ...
        string sql = "DELETE FROM RecetasCreadas WHERE ID_Recetas = @pID";
        using(SqlConnection db = new SqlConnection(_connectionString))
        {
